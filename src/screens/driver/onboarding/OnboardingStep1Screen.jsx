...
export default function OnboardingStep1Screen({ navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!nicNumber.trim())
      return Alert.alert("Error", "NIC number is required");
    if (!phoneNumber.trim())
      return Alert.alert("Error", "Phone number is required");
    if (!dateOfBirth) return Alert.alert("Error", "Date of birth is required");
        ...
        <View style={s.card}>
          ...
          <View style={s.field}>
            {(showDatePicker || !!dateOfBirth) && (
              <Text style={s.floatingLabel}>Date of Birth</Text>
            )}
            <TouchableOpacity
              style={s.dateBtn}
              onPress={() => setShowDatePicker(true)}
            >
              <Text style={[s.dateBtnText, !dateOfBirth && s.placeholder]}>
                {dateOfBirth
            ...
            <View style={s.optionsRow}>
              {WORKING_TIME_OPTIONS.map((o) => (
                <TouchableOpacity
                  ...
                  style={[
                    s.optionBtn,
                    workingTime === o.key && s.optionBtnActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      s.optionText,
                      workingTime === o.key && s.optionTextActive,
                    ]}
                  >
                    {o.label}
                  </Text>
