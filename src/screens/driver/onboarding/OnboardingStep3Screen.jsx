...
const DOCUMENT_TYPES = [
  ...
  { key: "revenue_license", label: "Revenue License", icon: "" },
];
...
export default function OnboardingStep3Screen({ navigation }) {
  ...
  const pickAndUpload = async (docType) => {
    ...
    if (status !== "granted") return Alert.alert("Permission needed", "Please grant media library permission");
    ...
    try {
      ...
      formData.append("document", { uri: file.uri, type: file.mimeType || "image/jpeg", name: file.fileName || `${docType}.jpg` });
    ...
    } catch (e) { Alert.alert("Error", "Upload failed. Please try again."); }
  ...
  const handleSubmit = async () => {
    ...
    if (uploaded.length < DOCUMENT_TYPES.length) {
      return Alert.alert("Error", `Please upload all ${DOCUMENT_TYPES.length} documents (${uploaded.length}/${DOCUMENT_TYPES.length} done)`);
    }
    setSubmitting(true);
    try {
      const token = await AsyncStorage.getItem("token");
      const docsArray = DOCUMENT_TYPES.map((d) => ({ documentType: d.key, documentUrl: documents[d.key] }));
  ...
  return (
    <SafeAreaView style={s.container}>
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <Text style={s.title}>Document Upload</Text>
        <Text style={s.subtitle}>Step 3 of 5  {uploadedCount}/{DOCUMENT_TYPES.length} uploaded</Text>
        <View style={s.card}>
          {DOCUMENT_TYPES.map((doc) => {
            const isUploaded = !!documents[doc.key];
            const isLoading = uploading[doc.key];
            return (
              <TouchableOpacity key={doc.key} style={[s.docBtn, isUploaded && s.docBtnDone]} onPress={() => pickAndUpload(doc.key)} disabled={isLoading}>
                <Text style={s.docIcon}>{doc.icon}</Text>
                <View style={s.docInfo}>
                  <Text style={[s.docLabel, isUploaded && s.docLabelDone]}>{doc.label}</Text>
          ...
          <View style={s.progressBar}>
            <View style={[s.progressFill, { width: `${(uploadedCount / DOCUMENT_TYPES.length) * 100}%` }]} />
          </View>
          <TouchableOpacity style={[s.nextBtn, (submitting || uploadedCount < DOCUMENT_TYPES.length) && s.btnDisabled]}
            onPress={handleSubmit} disabled={submitting || uploadedCount < DOCUMENT_TYPES.length}>
